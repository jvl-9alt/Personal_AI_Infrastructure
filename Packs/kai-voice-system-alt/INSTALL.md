...
## Pre-Installation: External Voice Server

This Pack supports running a local voice server or using an externally hosted server that implements the same API. The hooks and health checks will respect the `PAI_VOICE_SERVER` environment variable. If `PAI_VOICE_SERVER` is not set, the default is `http://localhost:8888`.

### Step 0.1: Configure `PAI_VOICE_SERVER`

Set `PAI_VOICE_SERVER` to the base URL of your voice server (omit `/notify`). Examples:

```bash
# Local (default)
export PAI_VOICE_SERVER=http://localhost:8888

# External hosted server
export PAI_VOICE_SERVER=https://voice.example.com
```

The `server-health-check.ts` utility included in this Pack uses `PAI_VOICE_SERVER` and will automatically normalize URLs (it strips trailing `/notify`).

If you prefer a different local port, point `PAI_VOICE_SERVER` to the custom port instead of using a VOICE_SERVER_PORT variable.
