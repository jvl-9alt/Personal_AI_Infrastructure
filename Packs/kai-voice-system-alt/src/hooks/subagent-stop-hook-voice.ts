...
import { enhanceProsody, cleanForSpeech, getVoiceId, getVoiceConfig, VoiceConfig } from './lib/prosody-enhancer';
import { getStdin } from './lib/get-stdin.ts';
...
async function main() {
  ...
  const voiceConfig = getVoiceConfig(finalAgentType) || undefined;
  console.log('Using voice ID for agent type', finalAgentType + ':', voiceId);
  console.log('Using voice config for agent type', finalAgentType + ':', voiceConfig ? JSON.stringify(voiceConfig) : 'none');
