...
export function getVoiceConfig(agentType: string): VoiceConfig | null {
  const paiDir = process.env.PAI_DIR || join(homedir(), '.config', 'pai');
  const voicesPath = join(paiDir, 'config', 'voice-personalities.json');
  console.log('Loading voice config from', voicesPath);
  function resolvePlaceholders(obj: any): any {
    if (typeof obj === 'string') {
      return obj.replace(/\$\{([A-Za-z0-9_]+)\}/g, (_, v) => process.env[v] || '');
    }
  ...
  try {
    if (existsSync(voicesPath)) {
      const raw = readFileSync(voicesPath, 'utf-8');
      const parsed = JSON.parse(raw);
      const voices = parsed && parsed.voices;
      if (voices && voices[agentType]) {
        const resolved = resolvePlaceholders(voices[agentType]);
        const voiceIdFromConfig = resolved.voice_id || resolved.voiceId || '';
        const voiceId = voiceIdFromConfig || getVoiceId(agentType) || '';
        return {
          voice_id: voiceId,
          voice_name: resolved.voice_name || resolved.voiceName || agentType,
          stability: Number(resolved.stability || 0),
          similarity_boost: Number(resolved.similarity_boost || 0),
          description: resolved.description || ''
        } as VoiceConfig;
      }
    }
    else{
      console.warn('Voice personalities config file not found at', voicesPath);
    }
  } catch (e) {
    console.error('Error loading voice config for agent type', agentType, e);
    // ignore parse/load errors and return null
  }
