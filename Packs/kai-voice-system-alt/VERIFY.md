...
```bash
curl ${PAI_VOICE_SERVER:-http://localhost:8888}/health
# Expected: a JSON health response with status, port and provider info
```

### Test 3: Test Voice Notification

```bash
curl -X POST ${PAI_VOICE_SERVER:-http://localhost:8888}/notify \
...
```bash
# Stop the voice server (best-effort)
pkill -f "voice/server.ts" || true
...
if [ -f "$PAI_CHECK/.env" ]; then
  if grep -q "ELEVENLABS_API_KEY" "$PAI_CHECK/.env"; then
    echo "✓ ELEVENLABS_API_KEY configured"
  fi
  if grep -q "GOOGLE_API_KEY" "$PAI_CHECK/.env"; then
    echo "✓ GOOGLE_API_KEY configured"
  fi
  if ! grep -q "ELEVENLABS_API_KEY\|GOOGLE_API_KEY" "$PAI_CHECK/.env"; then
    echo "⚠️  No TTS API key configured in $PAI_CHECK/.env (optional if using external hosted server)"
  fi
else
  echo "⚠️  No .env file found at $PAI_CHECK/.env (optional if using external hosted server)"
fi

echo ""

# Check voice server
if curl -s ${PAI_VOICE_SERVER:-http://localhost:8888}/health > /dev/null 2>&1; then
  echo "✓ Voice server responding at ${PAI_VOICE_SERVER:-http://localhost:8888}"
else
  echo "❌ Voice server NOT responding at ${PAI_VOICE_SERVER:-http://localhost:8888}"
fi
